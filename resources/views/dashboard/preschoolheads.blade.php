              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  @if (Auth::guard('web')->user()->promotion == 'Pre-School Head')
                    ...
                    @if ($view_preschoolstudent->centername == Auth::guard('web')->user()->centername && $view_preschoolstudent->status == null)
                      <tr>
                        <td>{{ $view_preschoolstudent->classname }}</td>
                        <td>{{ $view_preschoolstudent->middlename }}</td>
                        <td>{{ $view_preschoolstudent->fname }}</td>
                        <td>{{ $view_preschoolstudent->entrylevel }}</td>
                        <td>{{ $view_preschoolstudent->centername }}
                        <small>{{ $view_preschoolstudent->section }}</small>
                        </td>
  
                        <td><img style="width: 100%; height: 60px;" src="{{ URL::asset("/public/../$view_preschoolstudent->images")}}" alt=""></td>
                        <td><a href="{{ url('web/viewstudents/'.$view_preschoolstudent->ref_no) }}"
                      ...
                      <td>{{ $view_preschoolstudent->ref_no }}</td>
                        <td><a href="{{ url('web/editstudent/'.$view_preschoolstudent->ref_no) }}"
                          class='btn btn-info'>
                          <i class="far fa-edit"></i>
                      </a></td>  
                      
                        
                      <th><a href="{{ url('web/rejectstudent/'.$view_preschoolstudent->ref_no) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-user"></i>
                      </a></th>
                      <th><a href="{{ url('web/assignedteacher/'.$view_preschoolstudent->ref_no) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-comments"></i>
                      </a></th><th><a href="{{ url('web/suspendstudent/'.$view_preschoolstudent->ref_no) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-comments"></i>
                      </a></th>
  
                      <th> <a href="{{ url('web/studentsaddmit/'.$view_preschoolstudent->ref_no) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-user"></i> 
                      </a></th>
                      <th> <a href="{{ url('web/studentsubjects/'.$view_preschoolstudent->ref_no) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-user"></i> 
                      </a></th>
                    
                      {{-- <th><a href="{{ url('web/studentit/'.$view_preschoolstudent->ref_no) }}" class="btn btn-info"><i class="fas fa-user"></i> IT</a></th> --}}
                      <td><a href="{{ url('web/deletestudent/'.$view_preschoolstudent->ref_no) }}"

            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  @if (Auth::guard('web')->user()->promotion == 'High School')
                    @foreach ($view_highstudents as $view_highstudent)
                    @if ($view_highstudent->centername == Auth::guard('web')->user()->centername && $view_highstudent->status == null)
                      <tr>
                        <td>{{ $view_highstudent->classname }}</td>
                        <td>{{ $view_highstudent->middlename }}</td>
                        <td>{{ $view_highstudent->fname }}</td>
                        <td>{{ $view_highstudent->entrylevel }}</td>
                        <td>{{ $view_highstudent->centername }}
                        <small style="color: red">{{ $view_highstudent->section }}</small>
                        </td>
                      ...
                      <th><a href="{{ url('admin/rejectstudent/'.$view_highstudent->ref_no) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-user"></i>
                      </a></th>
                      <th><a href="{{ url('admin/assignedteacher/'.$view_highstudent->ref_no) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-comments"></i>
                      </a></th><th><a href="{{ url('admin/suspendstudent/'.$view_highstudent->ref_no) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-comments"></i>
                      </a></th>
  
                      <th> <a href="{{ url('admin/studentsaddmit/'.$view_highstudent->ref_no) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-user"></i> 
                      </a></th>
                      <th> <a href="{{ url('admin/studentsubjects/'.$view_highstudent->ref_no) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-user"></i> 
                      </a></th>
                    
                      {{-- <th><a href="{{ url('admin/studentit/'.$view_highstudent->ref_no) }}" class="btn btn-info"><i class="fas fa-user"></i> IT</a></th> --}}
                      <td><a href="{{ url('admin/deletestudent/'.$view_highstudent->ref_no) }}"

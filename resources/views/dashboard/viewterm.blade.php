              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    @foreach ($view_termtudents as $view_termtudent)
                      {{-- @if ($view_termtudent->section == Auth::guard('web')->user()->section &&  $view_termtudent->term == Auth::guard('web')->user()->term && $view_termtudent->classname == Auth::guard('web')->user()->classname && $view_termtudent->role == Auth::guard('web')->user()->role = null) --}}
                        <tr>
                          <td>{{ $view_termtudent->fname }}</td>
                          <td>{{ $view_termtudent->middlename }}</td>
                          <td>{{ $view_termtudent->surname }}</td>
                          <td> {{ $view_termtudent->section }}</td>
                          <td> {{ $view_termtudent->classname }}</td>
                          <td> {{ $view_termtudent->term }}</td>
                          <td> {{ $view_termtudent->gender }}</td>
                          <td><img style="width: 100%; height: 60px;" src="{{ URL::asset("/public/../$view_termtudent->images")}}" alt=""></td>
                          <td> <span class="badge badge-success">{{ $view_termtudent->status }}</span></td>
                          
                          {{-- <td><a href="{{ url('web/assignedstudent/'.$view_termtudent->ref_no1) }}"
                            class='btn btn-default'>
                             <i class="far fa-eye"></i>
                       --}}

                             <td><a href="{{ url('web/addresults/'.$view_termtudent->ref_no1) }}"
                                class='btn btn-info'>
                                 Add Results
                             </a></td>
                        </tr>
